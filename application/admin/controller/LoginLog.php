...
class LoginLog extends Controller {
    ...
    public function index(){
        ...
        $list = db('LoginLog')->where($where)->order('id desc')->paginate(20);
    ...
    public function save(){
        ...
        if($rows){
            $this->success(empty($id) ? '添加成功':'更新成功', url('/admin/Login_Log/index'), [], 0);
        }else{
    ...
    public function delete(){
        $id = input("id");
        $rows = LoginLogModel::destroy($id);
        $this->success("已成功删除 {$rows} 条记录.", url("/admin/Login_Log/index"), '', 0);
    }
}
