...
class NewsCate extends Controller {
    ...
    public function index(){
        ...
        $list = db('NewsCate')->where($where)->order('id desc')->select();
    ...
    public function edit(){
        $id = input("id");
        $Obj = NewsCateModel::get($id);
        $this->assign('Obj',$Obj);
        $data = db("news_cate")->select();
        $selectStr = makeSelectOption($data);
    ...
    public function save(){
        ...
        if($rows){
            $this->success(empty($id) ? '添加成功':'更新成功', url('/admin/News_Cate/index'), [], 0);
        }else{
    ...
    public function delete(){
        $id = input("id");
        $rows = NewsCateModel::destroy($id);
        $this->success("已成功删除 {$rows} 条记录.", url("/admin/News_Cate/index"), '', 1);
    }
}
