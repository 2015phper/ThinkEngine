 ...
 * @param type
 */
function refreshCapt($this, type=true) {
...
function getProvince(_this,status) {
    ...
    if(!status) {
        ...
        var index = layer.msg("确定要取消地区选择吗？",{time:0,btn:['确定','取消'],yes:function () {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ...
            success: {
                ...
                className: "btn-info",
                callback: function() {
                    window.location.href = '/admin/user/loginOut';
                }
            }
        }
    });
...
function delChecked(url, DOM, className) {
    ...
    var index = layer.confirm('确定要删除这'+count+'条记录吗？', {icon: 3, title:'提示'}, function(index){
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data.code == 1){
                    $.niftyNoty({type: 'success', message : data.msg, timer: 3000, onHidden:function () {
...
function delThis(url, $this) {
    if(!url){layer.msg("什么也没有删除！");return false;}
    bootbox.dialog({
        message: "确定要删除此条记录吗？",
        title: "数据删除",
        buttons: {
...
function uploadBase($this) {
    ...
    if(!url){layer.msg("截取错误，请稍后重试！",{icon:2});return false;}
...
function send(url ,$value) {
    if(!url){$.niftyNoty({message: "请配置您的更新地址~",type: "danger"});return false;}
